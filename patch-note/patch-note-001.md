...
#60 작업(v001.111.0) 후 마스터 검증 중 발견: 주간-스코프 KPI 카드 "전체 메시지"가 월요일(W23 시작일) 반나절치인 3987로 표시되어 "전체"라는 라벨 때문에 값이 틀려 보였다. 데이터·필터는 정상(W23 `by_day`=오늘 하루뿐, aggregate와 일치)이며 혼란의 원인은 **주간 값 카드에 'all-time'을 연상시키는 '전체' 라벨**이었다. 추가로 "메시지"의 의미(모델 API 응답 횟수, 도구 호출·서브에이전트 각 1건 집계)가 불명확했다.

### 페이즈 결과

- **HOTFIX 1 (Phase 2) — 주간-스코프 KPI 라벨 정정 + 메시지 투팁 (commit `9e223c6`)**: `monitoring/index.html`에서 `전체 메시지`→`주간 메시지`, `전체 토큰`→`주간 토큰`으로 변경(주간 스코프임을 라벨에 반영). `#kpi-messages-val`에 투팁(title) 추가 — "모델(어시스턴트) API 응답 횟수 — 도구 호출·서브에이전트·advisor 응답이 각각 1건으로 집계됨. 사람이 보낸 채팅 수가 아님." `모델 분포 (전체 토큰)`(토큰 종류 의미)와 캐시 설명 프로즈는 보존. +3/−3.
...
NOT APPLICABLE — 라벨/UX 정정만, 신규 규칙/훅/agent/스킬/검증축 추가 없음.
