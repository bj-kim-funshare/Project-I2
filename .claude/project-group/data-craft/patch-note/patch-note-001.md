...
- **`packages/fs-data-viewer/src/widgets/column-generator/available-types.ts`** 의 `AVAILABLE_COLUMN_TYPE_IDS` 배열에 `'rowLink'` 추가 (`'connection'` 다음).
- **`packages/fs-sub-data-viewer/src/widgets/column-generator/available-types.ts`** 동일 추가 (배열 끝).
- **`packages/fs-external-data-viewer/src/widgets/column-generator/available-types.ts`** 동일 추가.
- fs-sub-data-viewer / fs-external-data-viewer 의 `dist` 도 본 머지 후 재빌드 완료 (`'rowLink'` 가 새 dist 의 AVAILABLE 배열에 포함됨).

### 진단 정리 (실수 기록)

원 플랜 Phase 2 작성 시 `column-types` registry 만 확인하고 column-generator widget 의 `available-types.ts` allowlist 의 존재를 놓침. ColumnTypeButton 의 `isInAvailableList` 가드가 hard-cut 으로 작동하여 type 이 registry 에 있어도 화이트리스트 없으면 완전 숨김 — 본 second-layer filter 의 존재를 마스터의 반복 신고 후에야 ColumnTypeButton.tsx 직접 확인으로 발견. Phase 2-8 / HOTFIX 1 모두 이 파일을 검증 범위에 안 넣은 점이 직접적 원인.
...
- 코드 commit + i-dev 머지 후 sub/external dist 재빌드 통해 AVAILABLE 배열에 `'rowLink'` 포함 확인.
