...
v001.461.0(#169)에서 그룹핑 모드의 선택 행 삭제·열 일괄 입력 즉시 반영을 수정했으나, 후속으로 **개별 셀 편집** 경로의 잔여 결함이 보고됨: 행 그룹 기준 열에 대기/완료/진행중 3개 값이 있어 그룹이 3개일 때, 단일 셀에서 "대기" 행의 값을 "진행중"으로 바꾸면 진행중 그룹으로 이동해야 하는데 대기 그룹에 잔존하고 새로고침해야 이동하던 버그. 셀 편집은 `saveChange → updateCellInLoadedRow`(enterprise-492 HOTFIX-001) 경로로 흘러 셀 값만 in-place 패치되고 행이 그룹 간 이동을 하지 않은 것이 원인(Phase 2가 다룬 일괄 입력 경로와 동일 클래스의 다른 진입점).

### 페이즈 결과

- **핫픽스1 (fix, `0027a6f`)**: `updateCellInLoadedRow`(`useServerPagingOrchestrator.ts`)의 `setGroups` 콜백에 재그룹 분기 추가. 편집 열이 그룹 기준 열(`isGroupingActive && columnField === groupingColumnId`)이면 행을 현재 그룹에서 제거(`rowCount` 감소)하고 새 값(`newValue`)과 일치하는 그룹으로 이동(`rowCount` 증가), 대상 그룹이 없으면 신규 그룹 생성, 빈 그룹은 제거. **서버 재요청 없이 로컬 state 만 변경** — 저장 큐 flush 와의 비동기 경합을 회피(즉시·정확). 일반 열 편집은 기존 in-place 셀 패치 경로 무회귀.

### 검증

- data-craft lint gate(`pnpm typecheck:all && pnpm lint`) = 0 errors (잔존 경고 18건 베이스라인 동일).
- 수동: 상태 열로 행 그룹 활성화 → "대기" 그룹의 한 셀을 "진행중"으로 편집 → 새로고침 없이 즉시 진행중 그룹으로 이동, 대기 그룹 카운트 감소 확인. 일반 열 셀 편집 무회귀 확인.

### 알려진 후속 / 주의

- 본 핫픽스로 groups-store 동기화 결함의 3번째 인스턴스(notifyRowDeleted, 일괄입력 그룹기준열, 단일셀 그룹기준열)가 수정됨. 동일 클래스의 `notifyRowAdded`(행 추가)는 여전히 미수정 — 행 추가 미반영 발견 시 핫픽스2 대상.
- `group.rowCount`(서버 총개수) vs 로드분 정합 / 대상 그룹 내 정렬·페이지 위치는 다음 재조회에서 자기 교정(허용 caveat).
...
### 양측 보존 노트
- 본 항목 작성 시점에 main HEAD 가 동시 시점에 v001.318.0 (플랜 #133 HOTFIX 2) 을 선점하여 머지 충돌 발생. CLAUDE.md §5 step 4 양측 보존 원칙으로 본 항목을 v001.319.0 으로 승격, v001.318.0 (#133 HOTFIX 2) 은 아래에 그대로 보존.
