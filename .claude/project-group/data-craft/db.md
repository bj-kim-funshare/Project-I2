...
- **admin 전용 테이블** = **admin 콘솔/`data-craft-admin-server` 만 read/write** 하는 테이블. 예: `admin_email_verification`(고정 단일 계정 2단계 로그인 인증번호), `admin_promotion_audit`(관리자 행위 감사), `admin_internal_company_id` 등 관리 메타·감사·인증 객체. 신규 admin 전용 객체(테이블·제약·루틴)는 반드시 **`admin_` 접두**로 명명한다(아래 §배포 DB 검수 정책의 도메인 식별 근거 — 드리프트 게이트가 이 접두로 admin 도메인을 기계적으로 식별·제외한다).
...
- **구현 훅**(원칙 → wire): 도메인 제외 메커니즘 = `.claude/scripts/db-drift-domain.txt` 의 **`prefix:admin_` 규칙** — `admin_` 접두 명명 규율을 따르는 admin 도메인 전체를 게이트가 기계적으로 식별·제외하며, **신규 admin 객체 추가 시 정의 파일·게이트 무수정으로 자동 처리**(자가 유지)된다. 객체명 단위 exact 제외 줄 형식도 비-규약 env-specific 객체용으로 하위호환 지원. #379 false-block 은 이 접두 규율 부재로 admin 객체가 서비스 지문에 섞여 발생했다. **게이트 로직(서비스 도메인 한정 대조)의 실제 구현은 `plan-enterprise-os`(#63)가 완료**했으며(스크립트 `normalize_fp()` 의 `prefix:` 지시자 + `db-drift-domain.txt`), 본 절은 그 정책 근거다.
...
- **지문 정규화**: 거짓양성 억제를 위해 ① 시퀀스 현재값, ② 파티션 자동생성 child(예: `data_values` HASH(group_id) 8 서브파티션), ③ 통계/스토리지 메타, ④ **admin/관리 도메인 객체**(§"배포 DB 검수 정책 — 하드 규칙" — admin 전용 테이블은 dev 에만 존재하는 것이 설계상 정상이므로 드리프트 아님; `admin_*` 접두 명명 규약을 `db-drift-domain.txt` 의 `prefix:admin_` 규칙으로 제외)는 지문에서 제외하고 **서비스 도메인의** DDL 구조(테이블·컬럼·타입·제약·루틴 정의) 표면만 비교. 첫 실측에서 서비스 도메인 dev==prod(현재 동기 가정)여야 정상.
