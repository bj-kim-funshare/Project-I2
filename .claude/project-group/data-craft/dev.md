...
- **인증 — refresh token 채널**: 서버는 refresh token 을 `HttpOnly; Secure; SameSite=Lax` 쿠키로만 발급. localStorage / sessionStorage / IndexedDB 등 JS 접근 가능 영역 저장 금지. JSON response body 에 `refreshToken` 키를 포함하지 않는다 (signin / autoSignin / refresh rotation 전부).
...
- **인증 — origin 전략**: 모든 client origin 은 `datacraft.ai.kr` 동일 eTLD+1 아래로 통합 (same-site 쿠키 흐름 유지). `funshare-inc.github.io` 같은 cross-site origin 사용 금지. cross-site 필요가 생기면 `SameSite=None; Secure` 전환은 HTTPS 정합 검증 후 별도 결정.
