...
- **인증 — refresh token**: 서버가 `HttpOnly; Secure; SameSite=Lax` 쿠키로만 발급. localStorage / sessionStorage / IndexedDB 등 JS 접근 가능 영역 저장 금지.
- **인증 — access token**: 메모리 (모듈 스코프 또는 인메모리 스토어) 에만 보관. 보호된 API 호출은 `Authorization: Bearer` 헤더. request body 의 `accessToken` 키 사용 금지.
- **시크릿**: 자격 평문 (이메일·비밀번호 등) 을 소스 트리에 두지 않는다. dev 자동 로그인 편의는 `.env.local` 또는 동등한 gitignored 파일을 통해서만 주입. `.env.example` 은 키만 공개, 값은 공백.
...
- **새 창 열기**: `window.open` 호출 시 `'noopener,noreferrer'` 사용 의무. 반환 window 의 `opener=null` 방어 권장.
