...
targets:
  ...
  - name: data-craft-mobile
    ...
    port: 5174
    cache_paths: []
    lint_command: flutter analyze && pnpm typecheck
...
- **`data-craft-mobile` lint gate = `flutter analyze && pnpm typecheck` (하이브리드 repo — 2026-06-17 교정)**: 이 저장소는 네이티브 Flutter(`lib/`, `android/`, `ios/`)와 webview 탑재 React(`apps/web`, Node)가 공존한다. 기존 `pnpm typecheck` 단독은 apps/web 만 검사하고 네이티브 Flutter 코드를 전혀 게이팅하지 못했다(채팅 등 Flutter 작업이 게이트 없이 통과). `flutter analyze`(Flutter lib/ 정적분석)를 선행하고 `pnpm typecheck`(apps/web 타입체크)를 직렬 결합해 양쪽을 모두 검사한다. 둘 다 fresh 워크트리에서 각자 의존 설치(`flutter pub get` / `pnpm install`) 선행 필요.
