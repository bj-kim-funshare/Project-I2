...
11. **For each environment** (or single label in `공유` branch) in order:
    ...
    **b2. Capture rollback** (routine changes only — insert between connect and dry-run; greenfield 모드 시 skip — 기존 routine 없음):
...
Out of scope (v3):
- EVENT (MySQL) — scheduler dependency changes the capture/rollback semantics. v4 candidate, DEFER.
- DML (data manipulation) — `task-db-data` covers.
- Engines beyond MySQL/PostgreSQL (SQLite, MongoDB, etc.).
- Secret-manager-based credential resolution (env var only in v3).
- Framework migration runners (the skill calls SQL CLIs directly; `prisma migrate deploy` etc. are deferred).
- Protected-table enforcement (`db.md` lacks the `protected_tables` field in v3).
...
- **CREATE USER / GRANT** — user/privilege management is v4 candidate; master handles manually. DEFER.
