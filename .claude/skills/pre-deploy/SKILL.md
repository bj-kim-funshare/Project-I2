...
`.claude/md/branch-alignment.md` "Exit restoration" 절차 수행. 베이스 = `i-dev` (external context — 멤버 레포 기본 정렬). 실패 경로 (머지 충돌 등) 에서도 동일 복원 의무 — failure policy 의 각 행 처리 후 본 절차 수행.
