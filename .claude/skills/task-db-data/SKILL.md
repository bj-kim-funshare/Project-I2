...
11. **For each environment** (or single label in `공유` branch) in order:
    ...
    **c. Execute `capture.sql`** — substitute `__ENV_OR_LABEL__` → actual `env_or_label` before execution:
    ...
    ```
    This creates rollback tables named `_rollback_<table>_<env_or_label>_<execution_id>` in the target database.
    ...
    **f. Cleanup** — on success in this env: `DROP TABLE _rollback_*_${env_or_label}_<execution_id>` for every rollback table created in capture.sql (shell expansion at cleanup time — no token substitution needed here). On failure (after auto-rollback runs), preserve the rollback tables and report their names — master may need them for forensic review.
