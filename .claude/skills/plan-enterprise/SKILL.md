...
4. Current branch = `i-dev` (or `main` for bootstrap — `i-dev` is created from `main` HEAD on first invocation) (세부 절차: `.claude/md/branch-alignment.md` Entry verification — 본 스킬 컨텍스트 = external).
...
**Failure mode this prevents (verified — data-craft #338, 2026-06-16).** The stall is NOT the model "thinking it is done." In #338 the model explicitly narrated intent to continue ("코멘트 + Phase 6 디스패치를 함께 진행합니다") yet emitted only the per-phase-comment tool call as the turn's terminal `tool_use`; after that comment's tool result returned it re-engaged with nothing, so the turn completed with no pending action and the background job went idle. Therefore: when phases remain, the next-phase dispatch (7a) MUST be the next action after the comment's tool result — never let a per-phase comment be a turn's terminal tool call while phases remain. Background-job narration guidance ("report the next action when done") does NOT authorize ending the turn here; narrate inline and dispatch in the same drive.
...
After Step 10's merge succeeds, dispatch the `completion-reporter` sub-agent and halt. The plan issue, WIP branches (now merged), and i-dev state are all preserved as-is.

Dispatch `completion-reporter` with:
- `skill_type: "plan-enterprise"`
- `moment`: `"work_complete"` on first arrival (initial work complete); `"hotfix_complete"` when re-entering after a HOTFIX iteration.
- `data`: assemble per `.claude/md/completion-reporter-contract.md` §6 `plan-enterprise` schema.
  - For `work_complete`: required fields `leader`, `issue_number`, `issue_url`, `master_intent_summary`, `result_summary`, `phase_count`, `affected_files_total`; optional `solution_summary`, `manual_test_scenarios[]`, `next_action_guidance`, `post_action_hints[]`, `patch_note_version`, `advisor_plan_result`, `advisor_complete_result`.
...
2. Dispatch `completion-reporter` with:
   ...
   - `data`: assemble per `.claude/md/completion-reporter-contract.md` §6 `plan-enterprise` `skill_finalize` schema. Required: `leader`, `issue_number`, `issue_url`, `result_summary`, `total_phase_count`, `total_hotfix_count`, `wip_branches_merged[]`, `patch_note_version`, `patch_note_file`, `issue_close_status`, `worktree_cleanup_status`, `push_pending`; optional `push_status`, `advisor_plan_result`, `advisor_complete_result`, `phase_retry_count`.
