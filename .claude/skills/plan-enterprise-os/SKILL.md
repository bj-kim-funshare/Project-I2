...
Once `ExitPlanMode` is approved (Step 4), Steps 5 through 11 execute as **a single uninterrupted drive**. The main session does NOT end its turn at phase boundaries. This extends Step 4's "same response" guarantee to every phase boundary inside the Step 7 loop: immediately after a phase's per-phase verification ritual passes and its issue comment is posted, the **same turn** must continue with the next phase's `phase-executor` dispatch. The numbered/lettered sub-steps are checkpoints of one continuous pipeline, not discrete stopping points.

**The only legal yields (turn-ending points) are exactly three:**

- (a) a halt row of the Failure policy table or a per-phase verification cap-exhausted halt (phase iter budget reached),
