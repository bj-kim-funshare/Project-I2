...
```bash
# Self-detect: pnpm-workspace projects may have left dangling symlinks in the
# surrounding repo's node_modules after worktree removal (pnpm has no
# git-worktree awareness). Detect once and report; recovery is master-run.
# No-op if node_modules absent (I-OS harness / clean clone / npm·yarn projects).
if [ -d node_modules ]; then
  dangling=$(find node_modules -maxdepth 2 -type l ! -exec test -e {} \; -print 2>/dev/null)
  if [ -n "$dangling" ]; then
    count=$(printf '%s\n' "$dangling" | wc -l | tr -d ' ')
    echo "WARNING: ${count} dangling symlinks in $(pwd)/node_modules — run 'pnpm install' once to repair (skill procedure continues)."
  fi
fi
```

- 본 블록은 work repo cwd 에서 실행 — 머지/제거가 work repo working tree 에서 이루어지므로 `$(pwd)` 는 그 cwd 기준.
- 발견 시 절차 halt 하지 않음 — 머지/제거는 이미 완료, 복구는 master 가 `pnpm install` 1 회 실행으로 처리.
- I-OS harness (Project-I2) worktree 제거 시: 본 repo 에 `node_modules` 없음 → 첫 가드 false → no-op.
- npm/yarn 프로젝트: dangling symlink 구조 자체가 없음 → `find` 빈 결과 → 안내 없음.
...
**비권장**: worktree 내부에서 의존성을 건드리는 `pnpm` 명령 실행은 비권장 — 자가 검출이 사후 안내하지만, 불필요한 `pnpm install` 비용이 발생함.

**허용**: worktree 내부에서는 git 조작 + 코드 편집 + 정적 검사 (lint / typecheck — 메인이 이미 의존성을 깔아둔 상태로 실행) 만. dev 서버는 메인 repo cwd 에서만 띄운다.

**사고 사례**: 2026-05-28 data-craft #193 핫픽스3 — 74 개 dangling 발생. v001.107.0 이 외부 정책 분산 가이드로 1차 대응, v001.108.0 (이슈 #57) 이 worktree-lifecycle Remove 절차의 자가 검출로 근본 대응.
