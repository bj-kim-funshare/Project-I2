...
1. 후보 수집:
   `gh issue list --repo bj-kim-funshare/data-craft-server --state closed --limit 200 --json number,title,url,closedAt,comments`
2. 각 후보 이슈마다 `gh issue view <N> --repo bj-kim-funshare/data-craft-server --comments` 로 코멘트 전수 확인 → 위 3 조건 중 어느 하나라도 만족하면 대상에 포함.
