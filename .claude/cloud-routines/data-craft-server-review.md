...
| 항목 | 값 |
|---|---|
| 저장소 | `funshare-inc/data-craft-server` (Routine UI 의 repo 선택 박스에 직접 지정) |
...
또한 본 routine 은 `gh api` 로 자기 저장소(`funshare-inc/data-craft-server`)의 `i-dev` 브랜치에 있는 `.routine-state/safe-issues.json` 을 fetch 하여 안전 확정 이슈를 후보에서 제외한다. 이 파일은 `/review-check data-craft` 스킬이 안전 확정 시 자동 갱신 / 유지 관리하며 routine 자체는 read-only. 파일 또는 `i-dev` 가 아직 존재하지 않으면(초기 상태) routine 은 빈 안전-리스트로 진행 (정상 fallback) — `/plan-enterprise data-craft` 의 bootstrap (Project-I2 의 `.claude/cloud-routines/bootstrap-safe-issues-prompt.md` 참조) 으로 초기 생성 후 다음 cycle 부터 효율화.

## 지침 (Claude Desktop Routine 의 prompt 본문 — 이하 paste 대상)

```
당신은 funshare-inc/data-craft-server 리포의 클로즈 이슈에 대해 사후 다관점 코드 리뷰를 수행하는 루틴입니다. 본 실행은 읽기 전용이며 코멘트 게시 외 어떤 쓰기 동작도 금지됩니다.

# 사전 fetch — 안전-리스트

`gh api repos/funshare-inc/data-craft-server/contents/.routine-state/safe-issues.json?ref=i-dev` 호출하여 응답 본문의 `content` 를 base64 디코딩 후 JSON 파싱. `safe_issues[].number` 를 `SAFE_NUMBERS` 집합으로 보관. 404 (파일 부재) 또는 `i-dev` 부재 시 빈 집합으로 fallback (정상 동작).
...
1. 후보 수집:
   `gh issue list --repo funshare-inc/data-craft-server --state closed --limit 500 --json number,title,url,closedAt`
2. 후보에서 `SAFE_NUMBERS` 에 속한 이슈를 제외 (오픈은 --state closed 가 이미 제외 — 따라서 남은 제외 대상은 안전 확정 이슈뿐).
3. 각 후보 이슈마다 `gh issue view <N> --repo funshare-inc/data-craft-server --comments` 로 코멘트 전수 확인 → 위 3 조건 중 어느 하나라도 만족하면 대상에 포함.
...
10. 이슈에 새 코멘트 게시:
   `gh issue comment <N> --repo funshare-inc/data-craft-server --body-file <tmp>`
   코멘트 본문 형식 (Markdown):
