...
- 🟡 **뷰어 3포크 통합(#458)** — phase 1~4c i-dev 머지. **#459/#462/#463: print 비런타임 · #466: shared 레이어(config+lib 53파일)** base→core(매 증분 build+dev green·advisor PASS, Explore 119 안전closure 중 ~110 진행). 깨진 phase4d 폐기(origin ae85a2608). 잔여: grid/lib(60) → data-viewer/lib·aggregation → **E-namespace 본격이동(난관)** → 런타임 엔진/widgets/sub-external(런타임 검증 필요·천장).
- 🔴 **view-mode 팬아웃** — 미착수.
- 🔴 **프리셋·즐겨찾기** — 미착수.
