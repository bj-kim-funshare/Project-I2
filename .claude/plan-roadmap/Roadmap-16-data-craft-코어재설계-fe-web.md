...
- 🟡 **뷰어 3포크 통합(#458)** — phase 1~4c i-dev 머지. **#459/#462/#463: print 비런타임 레이어(타입·dev-alias·저장/캐시·lib유틸) base→core 사실상 완료**(매 증분 build+dev green·advisor PASS). 깨진 phase4d 폐기(origin ae85a2608). 잔여: 다른 안전 leaf 재스코프 → E-namespace 본격이동(난관) → 런타임 엔진/어댑터·widgets·sub/external(런타임 검증 필요).
- 🔴 **view-mode 팬아웃** — 미착수.
- 🔴 **프리셋·즐겨찾기** — 미착수.
