...
- **후속 4 — 미디어 네이티브 캡처(미디어 B)**: 인앱 **녹음(마이크)·녹화(카메라 캡처)** UI로 즉석 오디오/비디오 메시지 작성. 안드로이드 `RECORD_AUDIO`/`READ_MEDIA_IMAGES·VIDEO·AUDIO`(API33+)+`READ_EXTERNAL_STORAGE` fallback·iOS 마이크 usage 등 **미디어 권한 매니페스트** 추가. 본 단계는 **네이티브 권한·캡처라 Chrome 웹 검증 불가 → 실기기/에뮬 실측 전용**이라 본 로드맵 단계 목록에서 분리(2026-06-17, master 결정). 미디어 A(파일 선택 송수신, #356) 완료가 선행. `/plan-enterprise data-craft data-craft-mobile`.
