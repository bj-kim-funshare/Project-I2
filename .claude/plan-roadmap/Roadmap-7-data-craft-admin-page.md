...
🔴 (수동) admin-server `.env` 구성 — `ADMIN_EMAIL`/`ADMIN_PASSWORD_HASH`(일회용 노드 스크립트 bcrypt)/`ADMIN_JWT_SECRET`/`PG_*`(+`PG_*_PROD`)/`SMTP_*`/`DB_NAME_AUTH·DATA(_PROD)` 작성(`.env.example` 참조, **`.env` 미커밋** — git-tracked 정책이나 시크릿은 제외). admin-web `.env`(`VITE_ADMIN_API_BASE_URL`)는 기본값(localhost:8100)으로 충분. **dev 기동·트랙 C/D 검증·e2e 의 선행 1회 단계.**
...
실행 순서: 저장소 생성·그룹 등록(P1) → prod psql 에 `user_events` additive CREATE 마이그레이션(P2, 실행은 컷오버 윈도우 운영 게이트) · dev DB 테이블(P3) → 트랙 A(본체 인입)·트랙 B(admin 골격)는 저장소가 겹치지 않아 **병렬 그룹 1** → **(수동) admin-server `.env` 구성**(트랙 C/D dev 검증·e2e 선행 1회) → 트랙 C(프로모션 CRUD, admin 골격 의존) → 트랙 D(대시보드, 이벤트 데이터·admin 골격 의존) → 보안 점검 **병렬 그룹 2**.
