...
🟢 /plan-enterprise data-craft [R5·완료] 추천 마무리 2건 — (E5) referralDeduction `applyDeductionWithConnection`의 consumeCoupon 0행(charge 창에서 admin `revokeCoupon` race) 분기를 구조적 reconciliation record로 격상(`logger.error` 유지 + {couponId, paymentHistoryId, grossAmount, postCouponAmount, finalCharge}). charge가 tx 외부라 race prevent 불가→관측가능화, **backfill 없음**(consumed_payment_id NULL=revoke 시그니처·admin-read 보존), 멱등 기존 충족(revoke consumed_at!=NULL throw + consume WHERE consumed_at IS NULL). (E6-tail·마스터 결정 Option B) `referralEarning.service.ts:135` `|| relation.benefitsRevoked` → `&& !relation.benefitsRevoked` → 해지 시 추천인 10% tail 즉시 중단(full_phase 무영향). data-craft-server 2파일(referralDeduction.service.ts·referralEarning.service.ts), 코드-only, 2페이즈(af77ec9·72b17a3, 머지 08537ea), 이슈 #478, patch-note v001.1123.0, advisor 계획 BLOCK(초안 backfill이 revoke 시그니처 오염 지적)→관측가능화 리워크 PASS·완료 PASS(E6-tail 진리표 전수 검증). ※E5 정직: 빈 소비기록은 올바른 revoke 상태(referral_coupon에 revoked_at/status 없어 consumed_payment_id NULL이 유일 판별자). **Roadmap-17 전 14건 완결.**
