...
🔴 /task-db-data data-craft, prod MySQL→psql **일반 데이터 이관** changeset 작성 — **적재 방식: 소스 prod MySQL(`211.211.222.105:3306`, 레거시)을 read-only 분석 → prod psql(`data_craft_production`)에 동일엔진 DML 로 적재(읽기=MySQL 분석/쓰기=psql DML 이라 task-db-data 범위 안 — db.md 의 'cross-engine=plan-enterprise'는 두 엔진 직결 벌크전송을 가리킴, 본 방식 해당 안 됨). 소·중 테이블=INSERT changeset, `data_values`(대용량 EAV 값 테이블, dev 3.2M행)=`COPY` 벌크(MySQL 덤프→엔진변환→psql COPY). 소스 prod MySQL read-only 접속=Production Read 인가 필요.** **pre-flight 게이트(이관 차단): 소스 prod MySQL 에서 `SELECT count(*) FROM (SELECT value_id, group_id FROM data_values GROUP BY 1,2 HAVING count(*)>1) t` = 0 확인, 0 아니면 이관 중단. prod psql 은 `data_values` 파티션 PK `(value_id, group_id)` 를 강제하므로 중복쌍 존재 시 벌크 로드가 중도 실패함(dev `data_values_pkey` 는 INVALID·비강제였으나 prod 빌드 DDL 은 올바르게 강제 — 프롬프트 1 발견).** 엔진변환(uuid·enum 대소문자·smallint 0/1·jsonb·timestamp·IDENTITY 재설정) + column_type 오분류 등 prod 데이터 호환 교정 + 릴레이션 테이블 제외. (결제는 다음 프롬프트로 분리)
...
- **데이터 이관 방식 결정(2026-06-09)**: 일반·결제 이관은 **task-db-data 로 수행** — 소스 prod MySQL 을 read-only 분석(쓰기 아님)하고 prod psql 에 **동일엔진 DML(INSERT/COPY)** 로 적재하므로 task-db-data 범위 안. 대부분 테이블은 INSERT changeset, **`data_values`(대용량)만 `COPY` 벌크**(MySQL 덤프→엔진변환→psql COPY). 소스 prod MySQL(`3306`) read-only 접속은 실행 시 Production Read 인가 필요.
