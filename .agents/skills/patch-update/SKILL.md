...
The simple part. The protocol section below is boilerplate per `AGENTS.md` §5.
...
Standard pattern from `AGENTS.md` §5. Skill-specific values:
...
- **Commit message** (Korean per `AGENTS.md` §1) — `patch-update: patch-note-{next}.md 생성 ({target})`.
