...
Parse rule:
- If the first whitespace-separated token is a positive integer AND `Roadmap-<that-number>-*.md` exists under `.Codex/plan-roadmap/`, the invocation is **edit mode** for that roadmap.
...
3. `.Codex/plan-roadmap/` exists or can be created on first run (lazy-create per `README.md` §G design).
...
For create mode:
- List existing `Roadmap-*-*.md` under `.Codex/plan-roadmap/`. Parse leading integers. New `N` = max + 1, or 1 if none.
...
```bash
mkdir -p .Codex/plan-roadmap
```

Write the drafted content to `.Codex/plan-roadmap/Roadmap-{N}-{slug}.md`.
...
git add .Codex/plan-roadmap/Roadmap-{N}-{slug}.md
...
|------|-----|
| 모드 | 신규 / 편집 |
| 파일 | .Codex/plan-roadmap/Roadmap-{N}-{slug}.md |
| 프롬프트 수 | <count> (🔴 <n> / 🟡 <n> / 🟢 <n>) |
...
In scope:
- Create new roadmap files at `.Codex/plan-roadmap/Roadmap-{N}-{slug}.md` with the spec's status-icon + parallel-group format.
