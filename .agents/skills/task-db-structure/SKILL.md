...
1. `.Codex/project-group/<leader>/` exists with `db.md` populated.
...
14. **WIP → i-dev 자동 머지** (AGENTS.md §5 universal — preserve both on conflict, halt on mutually exclusive):
    ```bash
    git checkout i-dev
...
| Cause | Output |
|---|---|
| `.Codex/project-group/<leader>/db.md` not found | `"그룹 <leader> 에 db.md 부재 — /group-policy 실행 필요"` |
