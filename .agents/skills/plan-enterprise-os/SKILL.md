...
System-side counterpart of `plan-enterprise`. The harness itself is the target — `.Codex/` content (skills, agents, md, rules, hooks, scripts), root config files (`AGENTS.md`, `README.md`), and other harness machinery.
...
- No project-group manifest dependency. Verification context comes from `AGENTS.md`, `MEMORY.md` index + relevant memory files, and inspection-procedure / shared md files.
...
- `phase-executor` is reused — the `group_policy_summary` input is renamed/repurposed as `harness_context` (AGENTS.md hard-rule excerpts + relevant memory excerpts). The agent itself does not change; the dispatcher feeds it different input content.
...
1. cwd is the Project-I2 repo (AGENTS.md at root).
...
- `.Codex/skills/*/SKILL.md` for skill-relevant changes.
- `.Codex/agents/*.md` for sub-agent definitions.
- `.Codex/md/*.md` for shared procedures.
- `AGENTS.md` for hard-rule context.
...
| Harness Integrity | 본 계획이 AGENTS.md 의 hard rule (§1 언어 분리 / §2 메인 세션 읽기 전용 / §3 스킬 기반 / §5 WIP 머지 / §8 토큰 예산) 그리고 memory 의 행동 규범에 위반/충돌이 있는가? |
...
- Relevant AGENTS.md hard rules (which apply to this phase).
...
3. Patch-note path: `patch-note/patch-note-{NNN}.md` (repo root, NOT under `.Codex/`). Parse for max `K`; new entry = `v{NNN}.{K+1}.0`.
...
### Step 11 — PENDING gate (per `.Codex/md/completion-gate-procedure.md`)
...
In scope:
- Harness-side plans on this repo (skills, agents, md, rules, hooks, scripts, AGENTS.md, README, root configs).
