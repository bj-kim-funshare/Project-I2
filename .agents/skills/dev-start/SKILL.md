...
- `.Codex/project-group/<leader>/dev.md`
...
`.Codex/skills/dev-start` declares this format; `group-policy` must emit conformant manifests.
...
| Cause | Output |
|---|---|
| `.Codex/project-group/<leader>/` does not exist | `"그룹 <leader> 미등록 — /new-project-group 먼저 실행"` |
