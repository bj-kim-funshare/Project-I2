...
description: Register a new project group under .Codex/project-group/{leader}/. Collects four policy areas (dev / deploy / db / group) from master via structured prompts with free-text additions, validates the whole set with one advisor call, then creates the folder skeleton — Index.md, dev/deploy/db/group policy files, and an initial patch-note-001.md. Fails fast if the leader already exists (points to /group-policy for re-config).
---
...
1. `.Codex/project-group/<leader>/` must **not** already exist.
2. Every repository path must exist on disk.
3. Current branch = `i-dev` (or `main` for i-dev bootstrap), per `AGENTS.md` §5 convention used across the harness.
...
Fields per project: `cwd`, `dev_command`, `port`, `cache_paths`. These map 1:1 to the YAML schema declared in `.Codex/skills/dev-start/SKILL.md` §Manifest contract — that schema is the single source of truth; this skill emits conforming output.
...
4. **Language adherence** — free-text content is Korean per the `.Codex/project-group/` content language lock (release/operational artifact). YAML keys and closed-choice values stay Latin.
...
| Cause | Output |
|---|---|
| `.Codex/project-group/<leader>/` already exists | `"그룹 <leader> 이미 등록됨 — 정책 수정은 /group-policy 사용"` |
