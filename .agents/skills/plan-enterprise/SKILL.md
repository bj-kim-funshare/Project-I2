...
1. `.Codex/project-group/<leader>/` exists with at minimum `dev.md` (used to enumerate repos and identify the primary work repo).
...
3. Resolve target patch-note path: `.Codex/project-group/<leader>/patch-note/patch-note-{NNN}.md` where `NNN` is the highest existing number. Parse the file for max `K` in `## v{NNN}.K.0` headers; new entry is `v{NNN}.K+1.0`.
