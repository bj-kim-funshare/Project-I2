...
| `patch-confirmation` | 로컬 → origin push (push 전 미커밋 발생 시 patch-note 새 마이너로 흡수) |
...
| `task-db-structure` | DDL 변경 + greenfield 구축 (CREATE DATABASE / 다중 테이블 설계 / dev→staging→prod 자동 + 롤백) | db-migration-author |
...
| `advisor()` tool | `claude-opus-4-8` | `.claude/settings.json` `advisorModel` — API 가드: 메인 모델 이상 강도 필수 (Sonnet inversion 시도 2026-05-13 철회) |
| 7 read-only 리뷰어 (계획) | `claude-opus-4-8` | 200k 일반 (1M 아님). agent frontmatter `model:` 명시 |
...
**Effort 정책 (2026-05-13 lock)**: 13 sub-agent 전부 `effort: medium`. 각 agent frontmatter 의 `effort:` 필드로 선언 (Claude Code 공식 지원 필드, 값 `low`/`medium`/`high`/`xhigh`/`max`). 새 sub-agent 추가 시 명시적 예외 락이 없으면 동일.
...
```
.claude/
├── md/                      # 특화 룰 + 공용 절차 (skill/agent 참조로 on-demand 로드)
...
- **영문**: CLAUDE.md / .claude/ 하위 (md, skills, agents, scripts) 모든 spec 본문
...
### DB 작업
```
/task-db-structure <leader> [<issue#>]  # DDL / greenfield
...
- 산출물: 18 스킬 + 13 sub-agent + 1 공용 절차 + CLAUDE.md + patch-note 부트스트랩
